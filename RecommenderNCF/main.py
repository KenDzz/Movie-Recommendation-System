...
from RecommenderNCF.ncf import NCF


def import_data():
    np.random.seed(123)

    data = pd.read_csv('Movie-Recommender-Data/data-ncf/ratings.csv')
...
def get_farivoteMovie(user_id,links):
    subdata = import_data()
    filtered_data = subdata.loc[subdata['userId'] == user_id]
    dataFrameMovieMerge = pd.merge(filtered_data, links, on="movieId")
    return dataFrameMovieMerge




def get_recommendations_ncf(user_id, model , movies, links):
    subdata = import_data()
    ...
    return get_recommendations(subdata, test_data, user_id, model , all_movieIds , movies, links)


def get_recommendations(reviews, test_reviews, user_id, modelX, all_movieIds, movies, links):
    movies["movieId"] = movies["movieId"].astype(int)
    links["movieId"] = links["movieId"].astype(int)
    dataFrameMovieMerge = pd.merge(movies, links, on="movieId")
    movieImdbId = []
    ...
    for (u, i) in test_user_item_set:
        ...
        if u == int(user_id):
            ...
            selected_not_interacted = list(np.random.choice(list(not_interacted_items), 99))
            test_items = selected_not_interacted + [i]
            predicted_labels = np.squeeze(modelX(torch.tensor([u] * 100),
                                                torch.tensor(test_items)).detach().numpy())
            top10_items = [test_items[i] for i in np.argsort(predicted_labels)[::-1][0:10].tolist()]
            print(top10_items)
            for it in top10_items:
                recommendations += dataFrameMovieMerge.loc[movies['movieId'] == it, 'title'].item() + '\n'
                movieImdbId.append(dataFrameMovieMerge.loc[movies['movieId'] == it, 'imdbId'].item())
            print("Tested is:", dataFrameMovieMerge.loc[dataFrameMovieMerge['movieId'] == i, 'title'].item())
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if test == '':
            ...
            print("User Id:")
            x = input()
            #get_recommendations(subdata, test_data, x, model,all_movieIds)
